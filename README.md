...
| `TELEGRAM_BACKUP_NOTIFY` | — | Если `1` — тот же архив (включая `.env`) дополнительно уходит документом в Telegram: всем `ADMIN_IDS`, либо одним сообщением в группу с топиками, если настроена `ADMIN_GROUP_CHAT_ID` (см. «Уведомления в группу с топиками» выше, тема — `ADMIN_TOPIC_BACKUPS`) |
...
регулярно вручную. Учтите: архив, отправленный в Telegram
(`TELEGRAM_BACKUP_NOTIFY=1`), тоже содержит секреты из `.env` — Telegram
хранит документы на своих серверах, и обычные чаты с ботом (в отличие от
Secret Chats) не защищены end-to-end шифрованием.
